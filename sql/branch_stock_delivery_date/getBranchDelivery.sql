...
left outer join (
    ...
    (
        ...
        where
            counting_date <= ${date}
            and
            submission_time is not null
        group by
            product_id,
            branch_id) s1
            join branch_stock_delivery_date s2 on s1.product_id=s2.product_id and s1.branch_id = s2.branch_id and s1.counting_date = s2.counting_date
    ) last_count
    on
    ...
    where
        ...
        (
            ...
            or (s.real_delivery + s.product_count < s.min_stock and s.counting_date + 3 > ${date})
            or (s.real_delivery is null and s.counting_date + 3 > ${date})
        )
    ) aggreg
on
...
where
    ...
    and
        (bsr.date_rule <> ''
        or
        (bsr.date_rule is null and products.default_date_rule <> ''))
