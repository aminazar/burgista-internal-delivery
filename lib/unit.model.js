...
class Unit extends SqlTable {
  ...
  static select(isBranch, isKitchen) {
    ...
    if (isBranch === undefined) {
      ...
      if (typeof isKitchen !== 'undefined') {
        criteria.is_kitchen = isKitchen;
      }
      return new Promise((resolve, reject) => {
  ...
  static saveDateAfterLogin(name, username, isBranch, uid, isKitchen) {
    let curSql = Unit.test ? sql.test : sql;
    return new Promise((resolve, reject) => {
      return curSql[lastLoginTableName].get_previous_login_date({
        ...
        .then(() => Unit.afterLogin(name, username, isBranch, uid, isKitchen).then(resolve).catch(reject))
