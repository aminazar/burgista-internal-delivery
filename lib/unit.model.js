...
class Unit extends SqlTable {
  ...
  static async saveDateAfterLogin(name, username, isBranch, uid, isKitchen) {
    ...
    try {
      ...
      let login_date_time;
      if (!isBranch && !isKitchen) {
        const {_date} = Stock.getDate(Unit.dateTime, '00', '09');
        login_date_time = _date;
      } else {
        const {_date} = Stock.getDate(Unit.dateTime, '00', '01');
        login_date_time = _date;
      }
...
Unit.dateTime = moment().format('YYYY-MM-DD HH:mm:ss');
module.exports = Unit;
