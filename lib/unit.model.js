...
class Unit extends SqlTable{
  ...
  static saveDateAfterLogin(unitName, username, isBranch,uid) {
    let date = new Date();
    let curSql = Unit.test ? sql.test : sql;
    return new Promise((resolve, reject) => {
      ...
      .then(()=> {
        resolve({name: unitName, user: username, userType: username === 'admin' ? 'admin' : (isBranch ? 'branch' : 'prep')});
      })
