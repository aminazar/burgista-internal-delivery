...
*/
class SqlTable{
  constructor(tableName, idMember, test=false, testDate = moment().format('YYYY-MM-DD')){
    if(!tableName)
  ...
  save() {
    return new Promise((resolve, reject)=>{
      ...
      let execute = this[this.idMember] ? //Updates in case the model contains an id, otherwise adds data as new record
        //Update
        data => {
