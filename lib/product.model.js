...
class Product extends SqlTable {
  ...
  static select(username, unit_id, user_id){
      if (username !== 'admin' || typeof unit_id !== 'undefined') {
          return Product.selectByBranch(unit_id);
      }
  ...
  static async getAllProducts() {
    const curSql = Product.test ? sql.test : sql;
    try {
      let products = await curSql[tableName].getAll();
      return products;
    } catch (err) {
      console.error('an error occurred during fetching products', err)
    }
  }

  static selectByBranch(branchId) {
      let currentSql = Product.test ? sql.test : sql;
      return new Promise((resolve, reject) => {
          currentSql[tableName].getByBranchId({branch_id: branchId})
              .then((res) => {
  ...
  static selectWithPrepUnitData (username, uid, user_id) {
      ...
      let curSql = Product.test ? sql.test : sql;
      return new Promise((resolve, reject) => {
          curSql[tableName].getWithOverrides({uid})
              .then((res) => {
                  res.forEach( productData => {
                      for (let key in productData)
                          if ( productData.bsrid && productData[`default_${key}`] !== undefined && productData[key] !== null )
