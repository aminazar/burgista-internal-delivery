...
class Stock extends SqlTable {
  ...
  exportData() {
    ...
    const minusDay = helpers.timeCheck(d);

    if (this.real_delivery === undefined) {

      // Set date to yesterday unless time is after UPPER_BOUND_HOUR (in helper)
        ...
        this.sql.branch_stock_delivery_date.getProductWithOverride({pid: this.product_id, uid: this.branch_id})
          .then(res => {
            if (res.length !== 1)
              ...
              exprt.min_stock = Stock.calcMin(res[0], Stock.date, minusDay);
              resolve(exprt);
            }
          })
          .catch(err => {
  ...
  static select(user_id, date, unit_id) {
    let curSql = Stock.test ? sql.test : sql;
    let uid = typeof unit_id === 'undefined' ? user_id : unit_id;
    return curSql[tableName].selectMaxDate({date: moment(date).format('YYYY-MM-DD'), uid: uid})
  }
          ...
          product_ids.forEach(product_id => {
            ...
            if (coupleArray.length === 2) { // if couple found
              ...
              if (!couples[coupleArray[0].delivery_id]) { // check if this couple has been found
                ...
                let nextCountDate = rule.after(moment().toDate());
              ...
              let rule = RRule.fromString(ruleStr);
              row.next_count_date = rule.after(moment().toDate());
  ...
  static calcMin(productData, date, minusDay) {
