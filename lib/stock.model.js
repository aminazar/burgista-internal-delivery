...
class Stock extends SqlTable {
  ...
  static validationOnDate(date, time) {
    if (moment(`${time}`, 'HH:mm:ss').tz('Europe/London').isBefore(moment(`${timeLimit}`, 'HH:ss:mm').tz('Europe/London'))) {
      throw new Error('You are not allowed to register your stock before 10:30 pm');
    }
  }
  ...
  static select(user_id, date, unit_id) {
    ...
    return curSql[tableName].selectMaxDate({date: moment(date).format('YYYY-MM-DD'), uid})
      .then(res =>
        Promise.resolve(
          res
