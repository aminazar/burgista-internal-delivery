...
class Stock extends SqlTable {
  ...
  static deliverySelect(prep_id, unit_id, date, is_kitchen) {
    ...
    return new Promise((resolve, reject) => {
      Stock.branchStockDeliveryDateFunc(unit_id, true, moment(date).toDate(), is_kitchen)
        ...
        .then(res => {
          resolve(res.map(row => {
            const stockDate = row.submission_time ? moment(row.submission_time).toDate() : (row.counting_date ? moment(row.counting_date).toDate() : null);
            const rrule = row.date_rule !== null ? row.date_rule : row.default_date_rule;
            let untilNextCountingDay;
            try {
              untilNextCountingDay = rrule ? moment(RRule.fromString(rrule).after(moment().add(1, 'days').toDate())).diff(moment(), 'days') : null;
            } catch (e) {
  ...
  static rRuleCheckFunctionFactory(rRule, startDate, endDate) {
    ...
    return  date => list.find(el => moment(date).tz('Europe/London').format('YYMMDD') === moment(el).tz('Europe/London').format('YYMMDD'));
  }
          ...
          .then((res) => { // res is array of products
              ...
              while (countDate > previousLoginDate) { // goes back from today to the previous login date
                  stockStack = stockStack.concat( res.filter(r => r.default_date_rule &&
                      !stockStack.find(p => r.pid === p.pid) &&
                      Stock.rRuleCheckFunctionFactory(r.default_date_rule, previousLoginDate, moment(countDate).add(1, 'day').toDate())(countDate)));
