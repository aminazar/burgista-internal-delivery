...
class Stock extends SqlTable {
  ...
  exportData() {
    ...
    console.log('d: ', d);
    console.log('hour: ', momnet(d).format('HH'));

    const minusDay = parseInt(moment(d).format('HH')) < 6 ? 1 : 0;

    if (this.real_delivery === undefined) {

      // Set date to yesterday unless time is after 6 AM      
      exprt.submission_time = moment(d).day(d.day() - minusDay).format('YYYY-MM-DD hh:mm:ssZ');
    ...
    if (this[idColumn]) {
      ...
      // Set date to yesterday unless time is after 6 AM
      exprt.counting_date = moment(d).day(d.day() - minusDay).format('YYYY-MM-DD');
        ...
        this.sql.branch_stock_delivery_date.getProductWithOverride({pid: this.product_id, uid: this.branch_id})
          .then(res => {
            if (res.length !== 1)
              ...
              exprt.min_stock = Stock.calcMin(res[0], moment().utc().format('YYYY-MM-DD'), minusDay);
              resolve(exprt);
            }
          })
          .catch(err => {
  ...
  static deliverySelect(prep_id, unit_id, date) {
    let curSql = Stock.test ? sql.test : sql;
    return new Promise((resolve, reject) => {
      Stock.branchStockDeliveryDateFunc(unit_id, true, moment(date).toDate())
        .then(() => {
        ...
        .then(res => {
          resolve(res.map(row => {
            return {
              ...
              min: Stock.calcMin(row, moment(row.counting_date).toDate()),
  ...
  static calcMin(productData, date, minusDay = 0) {
    let overrideKey = `${moment(date).day(moment(date).day() -minusDay).format('ddd').toLowerCase()}_multiple`;
    let key = 'default_' + overrideKey;
    return Math.ceil((productData.min ? productData.min : productData.default_min) * (productData.usage ? productData.usage : productData.default_usage) * (productData[overrideKey] ? productData[overrideKey] : productData[key]));
  }
        ...
        return curSql.last_login.get_previous_login_date({login_uid: uid})
          .then((res) => {
            if (res.length) {
              ...
              if (previousLoginDate === null) previousLoginDate = moment(countDate).subtract(1, 'day').toDate();
            }
            else {
