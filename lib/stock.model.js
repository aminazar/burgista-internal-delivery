...
class Stock extends SqlTable {
  ...
  static select(user_id, date, unit_id) {
    let curSql = Stock.test ? sql.test : sql;
    const {_date} = Stock.getDate(date);

    let uid = typeof unit_id === 'undefined' ? user_id : unit_id;
    return curSql[tableName].selectMaxDate({date: _date, uid})
      .then(res =>
        Promise.resolve(
          res
  ...
  static getDate(date) {
    const clientTime = moment(date, 'YYYY-MM-DD HH:mm:ss').tz('Europe/London');
    const beforeTime = moment(`${Stock.date} 00:00:00`, 'YYYY-MM-DD HH:mm:ss').tz('Europe/London');
    const afterTime = moment(`${Stock.date} 01:00:00`, 'YYYY-MM-DD HH:mm:ss').tz('Europe/London');
    let returnDate;
    if (moment(clientTime).isBetween(beforeTime, afterTime)) {
      returnDate = moment(date, 'YYYY-MM-DD HH-mm-ss').subtract(1, 'day').tz('Europe/London').format('YYYY-MM-DD');
    } else {
      returnDate = moment(date, 'YYYY-MM-DD HH-mm-ss').format('YYYY-MM-DD');
    }
    return {
      _date: returnDate
    }
