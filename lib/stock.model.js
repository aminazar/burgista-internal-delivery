        ...
        return curSql.last_login.get_previous_login_date({login_uid: uid})
          ...
          .then((res) => { // res is array of products
              ...
              while (countDate > previousLoginDate) { // goes back from today to the previous login date
                stockStack.concat(res.filter(r => r.default_date_rule && Stock.checkIfToday(r.default_date_rule, countDate))
                  .forEach(el => {
                    ...
                    const foundProductInPrevDate = stockStack.find(p =>  el.pid === p.product_id && moment(p.counting_date).format('YYYY-MM-DD') > moment(countDate).format('YYYY-MM-DD'));

                    if (!foundProductInDate && !foundProductInPrevDate) {
                      stockStack.push({
                        product_id: el.pid,
                        branch_id: uid,
