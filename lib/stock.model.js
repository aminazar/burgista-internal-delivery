...
class Stock extends SqlTable {
  ...
  //   let dateTime = date;
  //   if (moment(date, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD') === moment(Stock.date).format('YYYY-MM-DD')) {
  //     dateTime = moment(`${Stock.dateTime}`, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss');
  //   }
  //   const {_date} = Stock.getDate(dateTime, {hour: '00', min: '00', second: '01'}, {hour: '01', min: '00', second: '00'});
  //   date = _date;
  //   try {
  //     let res = await curSql[tableName].selectMaxDate({date, uid});
  //     res = res.sort((x, y) => y.counting_date - x.counting_date)
  //              .filter((e, i, a) => a.findIndex(el => el.pid === e.pid) === i);
  //     res = await Stock.calculateMaxStock(res, uid, _date);
  //     return res;
  //   } catch (error) {
  //     throw error;
  //   }
  ...
  static async select(user_id, date, unit_id) {
    ...
    let productsToBeCountedInThisBranch = await curSql[tableName].getStockDeliveryBranch({date, uid});
    productsToBeCountedInThisBranch = productsToBeCountedInThisBranch.filter((e, i, a) => a.findIndex(el => el.bsddid === e.bsddid) === i);
  ...
  static async deliverySelect(prep_id, unit_id, date, is_kitchen) {
    ...
    const {_date} = Stock.getDate(dateTime, {hour: '00', min: '00', second: '00'}, {hour: '09', min: '00', second: '00'});
    try {
      if (_date === Stock.date) {
