...
class Stock extends SqlTable {
  ...
  static exportDataToSave(data) {
    ...
    if (data[idColumn]) {
      // In order to maintain a record of its true values, we must remove the extra values from the object
      // otherwise all data in history are removed
      if (data['ref_type_id'] === delivery_ref_types.CLOSED) {
        delete exprt.real_delivery;
        delete exprt.submission_time;
        delete exprt.product_count;
        delete exprt.is_delivery_finalised;
      }
      return exprt;
    }
    else {
      exprt.branch_id = data.branch_id;
      // when time is between 00:00:00 AM  into 12:00:00 need to 
      if (Stock.dateIsBetween('00:00:01', env.untilTime)) {
        exprt.counting_date = moment(Stock.date).subtract(1, 'day').format('YYYY-MM-DD');
      } else {
  ...
  static async checkStockOfBranchIsFinalized(branch_id, date) {
    let db = Stock.test ? env.testDb : env.db;
    const momentDate = moment(date).format('YYYY-MM-DD').toString();
    try {
      const result = await db.any(
        `select * from ${tableName} where counting_date = $1 and branch_id = $2 and is_delivery_finalised = $3`,
        [momentDate, branch_id, true]);
      if (result['length']) {
      ...
      console.error('an error occurred during checking stock of branch is finalized')
      throw error;
    }
  }
  ...
  static checkingSubmissionTime() {
    if (!Stock.dateIsBetween(env.fromTime, '23:59:59') && !Stock.dateIsBetween('00:00:01', env.untilTime)) {
      throw new Error('You are not allowed to register your stock, time is over');
    }
  }

  static batchCU(data, uid) {
    Stock.checkingSubmissionTime();
          ...
          db.tx(t => {
            ...
            if (oldItems.length > 0) {
              queries.concat(oldItems.map(item => {
                ...
                //FIXME: 
                // TypeError: Stock.exportDataToSave(...).then is not a function
  ...
  static async select(user_id, date, unit_id) {
    ...
    // 2) counted but not delivered (counting_date + 4 > date and counting_date < date )
    const productsCountedButNotDelivered = await curSql[tableName].countedButNotDelivered({ date, uid });
    // 3) not counted but delivered and finalized (counting_date + 4 > date and counting_date < date )
    const procutsNotCountedButDeliveredAndFinalized = await curSql[tableName].notCountedButDeliveredAndFinalized({ date, uid });
    // 4) not counted but not delivered (counting_date + 4 > date and counting_date < date )
    const productsNotCountedAndNotDelivered = await curSql[tableName].notCountedAndNotDelivered({ date, uid });
    // 5) delivered less than expected  (counting_date + 4 > date and counting_date < date )
    ...
    delivery_types['countedButNotDelivered'] = Stock.removeProductsMustBeCountedToday(productsCountingToday, productsCountedButNotDelivered, delivery_ref_types.COUNTED_BUT_NOT_DELIVERED, date);
    delivery_types['notCountedButDeliveredAndFinalized'] = Stock.removeProductsMustBeCountedToday(productsCountingToday, procutsNotCountedButDeliveredAndFinalized, delivery_ref_types.NOT_COUNTED_BUT_DELIVERED_AND_FINALIZED, date);
    delivery_types['notCountedAndNotDelivered'] = Stock.removeProductsMustBeCountedToday(productsCountingToday, productsNotCountedAndNotDelivered, delivery_ref_types.NOT_COUNTED_AND_NOT_DELIVERED, date);
    delivery_types['deliveredLessThanMinStock'] = Stock.removeProductsMustBeCountedToday(productsCountingToday, productsDeliveredLessThanMinStock, delivery_ref_types.DELIVERED_LESS_THAN_MIN_STOCK, date);

    // additional products, that imported by branch
    Stock.clearProductCountedTodayFromAdditonalProducts(delivery_types);
    ...
    let inventoriesMerged = [].concat(delivery_types['countingToday'],
      ...
      delivery_types['importsByBranch']
    );
    ...
    // for some additional we need remove duplicate product_id are occurred , when sorting base on counting_date
    inventoriesMerged = Stock.removeDuplicateItems(inventoriesMerged);
    
    return inventoriesMerged;
  }


  static clearProductCountedTodayFromAdditonalProducts(delivery_types) {
    // additional products, that imported by branch
    delivery_types['importsByBranch'] = delivery_types['countingToday'].filter(el => el['ref_type_id'] === delivery_ref_types.IS_NEW_PRODUCT);
    // clear product must be counted today from (additional products)
    delivery_types['countingToday'] = delivery_types['countingToday'].filter(el => el['ref_type_id'] !== delivery_ref_types.IS_NEW_PRODUCT);
  }

  static removeDuplicateItems(inventoriesMerged) {
    inventoriesMerged.sort((a, b) => (a.counting_date > b.counting_date) ? 1 : (a.counting_date === b.counting_date) ? ((a.counting_date > b.counting_date) ? 1 : -1) : -1 );
    const product_ids = [];
    const final_result = [];
    inventoriesMerged.forEach(inv => {
      const productDuplicateFound = product_ids.find(pId => pId === inv['pid']);
      if (!productDuplicateFound) {
        product_ids.push(inv['pid']);
        final_result.push(inv);
      }
    });
    return final_result;
  }
  ...
  static removeProductsMustBeCountedToday(todayStocks, stocks, type_id, date) {
    const temp_data = [];
    let stocks_unique = [];
    stocks.forEach(stock => {
    ...
    todayStocks.forEach(stock => {
      if (stock['ref_type_id'] && stock['ref_id'] && stock['ref_type_id'] === type_id && moment(stock['counting_date']).format('YYYY-MM-DD') ===  moment(date).format('YYYY-MM-DD')) {
        temp_data.push(stock);
      }
    });
    return temp_data;
  ...
  static async deliverySelect(prep_id, unit_id, date, is_kitchen, is_reporter) {
    let deliveries = [];
    try {
      /**
       * check when unit fetching stocks, is not reporter
       */
      if (!is_reporter) {
        await Stock.branchStockDeliveryDateFunc(parseInt(unit_id, 10), true, moment(date).toDate(), is_kitchen);
      }
      deliveries = await Stock.select(null, moment(date).format('YYYY-MM-DD'), unit_id);
    } catch (err) {
      console.error(err);
    }
    deliveries = Stock.deliveryItemsMapped(deliveries, unit_id, date);
    return deliveries;
  }

  static deliveryItemsMapped(deliveries, unit_id, date) {
    ...
    deliveries.forEach(delivery => {
      ...
      temp_deliveries.push({
        id: delivery.bsddid,
        uid: unit_id,
        counting_date: delivery['counting_date'],
        stock: delivery.product_count,
        stockDate,
        isCounted: moment(delivery['submission_time']).format('YYYY-MM-DD') === moment(date).format('YYYY-MM-DD'),
      ...
      if (is_branch) {
        return curSql.last_login.get_previous_login_date({login_uid: uid})
          ...
          .then((res) => { // res is array of products
              const {stockStack} = Stock.getStockStackBaseOnTwoLogin(res, countDate, previousLoginDate, uid);
  ...
  static getStockStackBaseOnTwoLogin(products, countDate, previousLoginDate, uid) {
    let data = [];
    let step = 0;
    while (countDate > previousLoginDate) { // goes back from today to the previous login date
      data.concat(products.filter(r => r.default_date_rule && Stock.checkIfToday(r.default_date_rule, countDate))
        .forEach(el => {
          // checking counting_date is today ref_type_id = null, otherwise set ref_type_id = NOT_COUNTED_AND_NOT_DELIVERED
          // set ref_type_id = NOT_COUNTED_AND_NOT_DELIVERED, means Products should be counted in earlier days
          //FIXME: 
          // we can't use this code that commented, might server time = 2019-01-02 (time between 00:00:00 and 10:00:00) and counting_date = 2019-01-01
          // because branch can inventory taking tomorrow between (00:00:00 until 10:00:00)
          /*let ref_type_id = null;
          const serverTime = moment().tz('Europe/London').format('YYYY-MM-DD');
          // const countingDate = moment(countDate).format('YYYY-MM-DD');
          if (countingDate === serverTime) {
            ref_type_id = delivery_ref_types.NOT_COUNTED_AND_NOT_DELIVERED;
          } */
          data.push({
            product_id: el.pid,
            branch_id: uid,
    ...
    return {
      stockStack: Stock.clearDuplicateProductsInDays(data)
    }
  }

  static clearDuplicateProductsInDays(stocks) {
    const data = [];
    const temp_stocks = [...stocks];
    const temp_stock_sorted = temp_stocks.sort((a, b) =>  new Date(b.date) - new Date(a.date));
    const product_branch_ids = [];
    temp_stock_sorted.forEach(stock => {
      const productFound = product_branch_ids.find(el => el['product_id'] === stock['product_id'] && el['branch_id'] === stock['branch_id']);
      if (!productFound) {
        product_branch_ids.push({
  ...
  static removeStock(bsddid) {
    let curSql = Stock.test ? sql.test : sql;
    try {
      return curSql['branch_stock_delivery_date']['removeById']({id: bsddid});
    } catch (error) {
      console.error('an error on delete record from branch_stock_delivery_date', error);
    }
  ...
  static checkingTime(isBranch) {
    if (Stock.dateIsBetween(env.untilTime, env.fromTime)) {
      return Promise.resolve(false);
    }
    return Promise.resolve(true);
  }
