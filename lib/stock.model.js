...
class Stock extends SqlTable {
  ...
  static async calculateMaxStock(res, uid, date) {
    ...
    try {
      const products = await curSql[tableName].getMaxCountProductInBranch({product_ids: product_ids, uid});
      products.forEach(product => {
        product['stockMax'] = Stock.calcMin(product, date, true);
      });
      res.forEach(el => {
  ...
  static async select(user_id, date, unit_id) {
    ...
    // set delivery type
    Stock.setDeliveryType(delivery_types.countedButNotDelivered, delivery_ref_types.COUNTED_BUT_NOT_DELIVERED, date);
    Stock.setDeliveryType(delivery_types.notCountedButDeliveredAndFinalized, delivery_ref_types.NOT_COUNTED_BUT_DELIVERED_AND_FINALIZED, date);
    Stock.setDeliveryType(delivery_types.notCountedAndNotDelivered, delivery_ref_types.NOT_COUNTED_AND_NOT_DELIVERED, date);
    Stock.setDeliveryType(delivery_types.deliveredLessThanMinStock, delivery_ref_types.DELIVERED_LESS_THAN_MIN_STOCK, date);


    // clear today counting
    delivery_types['countingToday'] = delivery_types['countingToday'].filter(el => !el['ref_id'] && !el['ref_type_id']);

    // set some information on stocks
    Object.entries(delivery_types).forEach(el => {
      const [key, value] = el;
      Stock.setLastCountingDate(value, LastDateToCountTheProductsInTheBranch, date);
      Stock.setUntilNextCountingDay(value);
    });
    const deliveryConcat = [].concat(delivery_types['countingToday'],
  ...
  static setLastCountingDate(stocks, products) {
    ...
    stocks.forEach(stock => {
      ...
      stock['min'] = Stock.calcMin(stock, stock.counting_date),
      stock['max'] = Stock.calcMax(stock, stock.counting_date),
  ...
  static setDeliveryType(stocks, type_id, date) {
    ...
    Stock.setAdditionalInformation(stocks, date, type_id);
  }
  ...
  //   const curSql = Stock.test ? sql.test : sql;
  //   const uid = typeof unit_id === 'undefined' ? user_id : unit_id;
  //   let res = await curSql[tableName].selectMaxDate({date: moment(date).format('YYYY-MM-DD'), uid});
