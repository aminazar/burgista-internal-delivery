...
class Stock extends SqlTable {
  ...
  static async select(user_id, date, unit_id) {
    ...
    return res.sort((x, y) => y.counting_date - x.counting_date)
       .filter(el => moment(el['counting_date']).format('YYYY-MM-DD') <= moment(_date).format('YYYY-MM-DD'))
  ...
  static getDate(date) {
    ...
    const beforeTime = moment(`${Stock.date} 00:00:00`, format).format(format);
    const afterTime = moment(`${Stock.date} 01:00:00`, format).format(format);
    let returnDate;
