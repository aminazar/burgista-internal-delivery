...
class Stock extends SqlTable {
  ...
  static async calculateMaxStock(res, uid, date) {
    ...
    try {
      ...
      if (product_ids['length']) {
        products = await curSql[tableName].getMaxCountProductInBranch({product_ids: product_ids, uid});
      }
      if (products['length']) {
  ...
  static async select(user_id, date, unit_id) {
    ...
    inventoriesMerged = await Stock.calculateMaxStock(inventoriesMerged, uid, date);
