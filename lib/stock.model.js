...
let idColumn = 'bsddid';
const timeFormatter = d => new Date( moment(d).format('YYYY-MM-DD') + 'T00:00:00Z');
const stockColumns = [
  'product_id',
  'branch_id',
  'counting_date',
...
class Stock extends SqlTable {
  ...
  static rRuleCheckFunctionFactory(rRule, startDate, countDate ) {

    let r = RRule.fromString(rRule);
    r.options.dtstart = new Date('2019-01-01T00:00:00Z');
    let list = r.between(timeFormatter(moment(startDate).add(1,'day').toDate()), timeFormatter(moment(countDate).add(1,'day').toDate())).map(r => moment(r).format('YYMMDD'));
    let needle = moment(timeFormatter(countDate)).format('YYMMDD');
    return list.find(l => l === needle);
  }
      ...
      if (is_branch) {
        return curSql.last_login.get_previous_login_date({login_uid: uid})
          ...
          .then((res) => { // res is array of products
              let stockStack = [];
              console.log(res.find(p=>p.pid===118), previousLoginDate, countDate)
              while (countDate > previousLoginDate) { // goes back from today to the previous login date
                stockStack = stockStack.concat(res.filter(r => r.default_date_rule &&
                  ...
                  Stock.rRuleCheckFunctionFactory(r.default_date_rule, previousLoginDate, countDate))
                  .map(el => {
                    return {
                      product_id: el.pid,
                      branch_id: uid,
