...
class Stock extends SqlTable {
  ...
  static getDate(date) {
    const clientTime = moment(date).tz('Europe/London').format('YYYY-MM-DD HH:mm:ss');
    const beforeTime = moment(`${Stock.date} 00:00:00`).tz('Europe/London').format('YYYY-MM-DD HH:mm:ss');
    console.log({beforeTime});
    const afterTime = moment(`${Stock.date} 01:00:00`).tz('Europe/London').format('YYYY-MM-DD HH:mm:ss');
    date = moment(date).tz('Europe/London').format('YYYY-MM-DD')
    if (moment(clientTime).isBetween(beforeTime, afterTime)) {
      date = moment(date).subtract(1, 'day').tz('Europe/London').format('YYYY-MM-DD');
    }
    return {
      _date: date
    }
