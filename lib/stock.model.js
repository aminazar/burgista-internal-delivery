...
class Stock extends SqlTable {
  ...
  static deliverySelect(unit_id, date) {
    let curSql = Stock.test ? sql.test : sql;
    return new Promise((resolve,reject) => {
      curSql.branch_stock_delivery_date.getBranchDelivery({date: moment(date).format('YYYY-MM-DD'),uid:unit_id})
        .then( res => {
          console.log(res);
          resolve(res.map(row => {
            return {
              ...
              stockDate: moment(row.submission_time).toDate(),
