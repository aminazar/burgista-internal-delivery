...
class Stock extends SqlTable {
  ...
  exportData(){
    ...
    exprt.submission_time = moment().utc().format();
    exprt.product_count = this.count;

    if(this[idColumn]) {
      exprt[idColumn] = this[idColumn];
      return exprt;
    }
    else {
      exprt.counting_date = moment().utc().format('YYYY-MM-DD');
      exprt.product_id = this.pid;
      return Product.select('admin', uid)({pid:this.pid})
        .then(res=>{
          exprt.min_stock = Stock.calcMin(res,moment().utc().format('YYYY-MM-DD'));
          resolve(exprt);
        })
        .catch(err=>{
          throw(err);
