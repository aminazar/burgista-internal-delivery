...
class Stock extends SqlTable {
      ...
      if (is_branch) {
        return curSql.last_login.get_previous_login_date({login_uid: uid})
          ...
          .then((res) => { // res is array of products
              const {stockStack} = Stock.getStockStackBaseOnTwoLogin(res, countDate, previousLoginDate);
  ...
  static getStockStackBaseOnTwoLogin(products, countDate, previousLoginDate) {
    let data = [];
    let step = 0;
    ...
    return {
      stockStack: Stock.clearDuplicateProductsInPreviousDays(data)
    }
  }
