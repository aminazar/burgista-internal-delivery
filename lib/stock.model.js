...
class Stock extends SqlTable {
  ...
  static async select(user_id, date, unit_id) {
    ...
    const {_date} = Stock.getDate(date, {hour: '00', min: '00', second: '01'}, {hour: '01', min: '00', second: '00'});
  ...
  static async deliverySelect(prep_id, unit_id, date, is_kitchen) {
    let curSql = Stock.test ? sql.test : sql;
    if (moment(date, 'YYYY-MM-DD HH:m:ss').format('YYYY-MM-DD') === moment(Stock.date).format('YYYY-MM-DD')) {
      date = moment(`${Stock.date} ${Stock.time}`).format('YYYY-MM-DD HH:mm:ss');
    }
    const {_date} = Stock.getDate(date, {hour: '00', min: '00', second: '00'}, {hour: '09', min: '00', second: '00'});
    try {
      if (_date === Stock.date) {
