            ...
            currentSql.branch_stock_delivery_date.inventoryCountingReport()
                .then(result => {
                    let filtered = result.filter(row => {
                        return parseInt(row.branch_id, 10) === parseInt(branchId, 10);
                    });
                    let couples = {}; // this object is just for preventing a couple to be added twice
                    let finalRows = [];
                    filtered.forEach((row, index, array) => {
                        let couple = array.find(item => { // finding couples
                          return row.product_code === item.product_code && row.delivery_id !== item.delivery_id;
                        });
                        if (couple) { // if couple found
                            let coupleArray = [row, couple];
                            ...
                            if (!couples[coupleArray[0].delivery_id]) { // check if this couple has been found
                                ...
                                let ruleStr = coupleArray[0].overridden_rrule ? coupleArray[0].overridden_rrule : coupleArray[0].rrule;
                                ...
                                let daysFromLastCounting = moment().diff(moment(coupleArray[0].counting_date), 'days');
                                ...
                                finalRows.push({
                                    ...
                                    next_count_date: rule.after(moment().toDate()),
