...
class Stock extends SqlTable {
  ...
  static rRuleCheckFunctionFactory(rRule, startDate, countDate ) {
    ...
    let list = r.between(moment(startDate).add(1, 'day').toDate(), moment(countDate).add(1, 'day').toDate()).map(r => moment(r).format('YYMMDD'));
    let needle = moment(countDate).format('YYMMDD');
