...
class Stock extends SqlTable {
  ...
  exportData() {
    ...
    if (this.real_delivery === undefined) {
      exprt.submission_time = moment().day(Stock.date.day()).month(Stock.date.month()).year(Stock.date.year()).format('YYYY-MM-DD hh:mm:ssZ');
        ...
        return curSql.last_login.get_previous_login_date({login_uid: uid})
          .then((res) => {
            if (res.length) {
              ...
              console.log('cp date',countDate,previousLoginDate);
            }
